...
const DAILY_CHALLENGES: DailyChallenge[] = [
  {
    ...
    description:
      "Take 3 slow, deep breaths right now and notice how your body feels.",
    reward: "+20 Karma",
    karmaValue: 20,
    type: "mindfulness",
  },
  {
    ...
    description:
      "Think of one small thing that went well today, no matter how tiny.",
    reward: "+15 Karma",
    karmaValue: 15,
    type: "wellness",
  },
  {
    ...
    title: "Posture Check",
    description:
      "Straighten your back and release the tension in your shoulders.",
    reward: "+10 Karma",
    karmaValue: 10,
    type: "wellness",
  },
  {
    ...
    description:
      "Drink a full glass of water right now. Your mood and brain will thank you.",
    reward: "+10 Karma",
    karmaValue: 10,
    type: "wellness",
  },
  {
    ...
    title: "Mindful Walk",
    description:
      "Take a 5-minute walk outside — no phone, just notice what's around you.",
    reward: "+25 Karma",
    karmaValue: 25,
    type: "action",
  },
  {
    ...
    description:
      "Write down one worry on paper, then fold it away. Let it go with intention.",
    reward: "+20 Karma",
    karmaValue: 20,
    type: "mindfulness",
  },
  {
    ...
    description:
      "Hold a genuine smile for 30 seconds. Science says it shifts your mood.",
    reward: "+10 Karma",
    karmaValue: 10,
    type: "wellness",
  },
  {
    ...
    title: "4-7-8 Breath",
    description:
      "Breathe in for 4s, hold for 7s, breathe out for 8s. Repeat 3 times. Instantly calming.",
    reward: "+20 Karma",
    karmaValue: 20,
    type: "mindfulness",
  },
  {
    id: "stretch_1",
    title: "Body Scan Stretch",
    description:
      "Stand up, reach your arms wide, and stretch every muscle group for 2 minutes.",
    reward: "+15 Karma",
    karmaValue: 15,
    type: "wellness",
  },
  ...
  },
  {
    ...
    title: "Screen Reset",
    description:
      "Look away from all screens and focus on something 20 feet away for 20 seconds.",
    reward: "+10 Karma",
    karmaValue: 10,
    type: "wellness",
  },
  {
    ...
    description:
      'Say out loud: "I am capable, I am growing, and today I choose peace."',
    reward: "+15 Karma",
    karmaValue: 15,
    type: "mindfulness",
  },
  {
    id: "music_1",
    title: "Mood Reset Song",
    description:
      "Play your favourite uplifting song and let yourself feel it completely.",
    reward: "+20 Karma",
    karmaValue: 20,
    type: "wellness",
  },
  {
    id: "meditation_1",
    title: "2-Minute Stillness",
    description:
      "Close your eyes and sit in complete silence for 2 minutes. Notice the peace.",
    reward: "+20 Karma",
    karmaValue: 20,
    type: "mindfulness",
  },
...
const COACHING_TIPS: CoachingTip[] = [
  ...
  },
  {
    ...
    category: "Self-Care",
    content:
      "Hydration impacts your mood more than you think. Have you had a glass of water recently?",
  },
  {
    id: "tip_3",
    category: "Focus",
    content:
      "If you're feeling overwhelmed, try the 5-4-3-2-1 technique to ground yourself in the present moment.",
  },
  {
    id: "tip_4",
    category: "Sleep",
    content:
      "Quality sleep is the single most impactful thing you can do for your mental health. Aim for a consistent bedtime.",
  },
  {
    id: "tip_5",
    category: "Mindfulness",
    content:
      "You cannot pour from an empty cup. Rest is not laziness — it's maintenance.",
  },
  {
    id: "tip_6",
    category: "Growth",
    content:
      "Progress, not perfection. One small positive action today compounds into major change over the next month.",
  },
  {
    id: "tip_7",
    category: "Anxiety",
    content:
      "Box breathing (4s in, hold, 4s out, hold) activates your parasympathetic nervous system in under 60 seconds.",
  },
];

class DashboardService {
  private getWellnessKarma(
    type: "breathing" | "meditation",
    duration: number,
  ): number {
    // Keep rewards consistent with current UX copy.
    if (type === "meditation") return 30;
    return 5;
  }
    ...
    try {
      ...
      if (memory.recent_events && memory.recent_events.length > 0) {
        return memory.recent_events[
          Math.floor(Math.random() * memory.recent_events.length)
        ];
      }
      return null;
    } catch (error) {
  ...
  async completeChallenge(userId: string, challengeId: string, reward: string) {
    try {
      ...
      if (error) {
        logger.error(
          "DashboardService: completeChallenge insert failed",
          error,
        );
        return false;
      }

      // Persistence: Increment karma in profiles table
      ...
      if (profileError) {
        // Fallback: If RPC doesn't exist yet, try direct update (less atomic but works)
        logger.warn(
          "DashboardService: increment_karma RPC failed, trying direct update",
          profileError,
        );
        const { data: currentProfile } = await (supabase as any)
          ...
          .eq("user_id", userId)
          .single();
        const newKarma = (currentProfile?.karma || 0) + karmaValue;
  ...
  async logWellnessSession(
    ...
    try {
      ...
      const alreadyAwarded = await this.isChallengeCompletedToday(
        userId,
        challengeId,
      );
      const karmaValue = this.getWellnessKarma(type, duration);
      ...
      if (!alreadyAwarded) {
        ...
        // Persistence: Increment karma in profiles, fallback to direct update.
        const { error: profileError } = await (supabase as any).rpc(
          "increment_karma",
          {
            p_user_id: userId,
        ...
        if (profileError) {
          logger.warn(
            "DashboardService: increment_karma RPC failed in wellness flow, trying direct update",
            profileError,
          );
          const { data: currentProfile } = await (supabase as any)
            .from("profiles")
  ...
  async getUserStats(userId: string) {
    try {
      // First try to get cached karma from profile for speed
      ...
      const profileKarma = profData?.karma || 0;
  ...
  private _calculateStreak(entries: Array<{ completed_at: string }>): number {
    ...
    const uniqueDays = [
      ...new Set(
        entries.map((e) => e.completed_at?.split("T")[0]).filter(Boolean),
      ),
    ]
      .sort()
  ...
  getEmpathyMessage(intensity: number, companionName: string): string {
    if (intensity >= 8)
      return `You've been doing great lately, I love seeing you so happy! ✨`;
    if (intensity >= 6)
      return `I'm here if you want to chat about anything on your mind. 🍵`;
    if (intensity >= 4)
      return `Everything's going to be okay. Remember to breathe. 🕊️`;
